...
class Photo
{
    int Width { get; set; }
    int Height { get; set; }
    protected double price8X10 { get; }
    protected double price10x12 { get; }
    protected double priceOther {  get; }
}
class PhotoPrice
{
...
Include a `protected` `price` field, and set it to $3.99 for an 8-inch by 10-inch photo, $5.99 for a 10-inch by 12-inch photo, 
and $9.99 for any other size (because custom cutting is required). The `price` field requires a get accessor but no set accessor.
...
The `MattedPhoto` `class` includes a `string` field named `Color` to hold a color, and the `FramedPhoto` `class` includes two `string` fields 
that hold the frame’s material (such as silver) and style (such as modern), named `Material` and `Style` respectively. 
...
```
> In order to prepend the *$* to currency values, the program will need to use the `CultureInfo.GetCultureInfo` method. In order to do this, 
include the statement `using System.Globalization;` at the top of your program and format the output statements as follows: 
